function sequence_get_animation_origin(_seq, _name, _t)
{
	if ( !is_struct(_seq) ) _seq = sequence_get(_seq);
    var track = sequence_get_animation_track(_seq.tracks, _name);
    if (!is_struct(track)) return [0,0];

    var sub;
    for (var i = 0; i < array_length(track.tracks); i++)
        if (track.tracks[i].name == "origin") { sub = track.tracks[i]; break; }

    if (!is_struct(sub)) return [0,0];

    var keys = sub.keyframes;
    if (array_length(keys) == 0) return [0,0];
